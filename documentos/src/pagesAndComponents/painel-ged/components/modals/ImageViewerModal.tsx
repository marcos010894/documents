...
import { TfiClose, TfiZoomIn, TfiZoomOut, TfiDownload } from 'react-icons/tfi';
...
const ImageViewerModal: React.FC<ImageViewerModalProps> = ({
    ...
    const imageUrl = file.url || `/api/documents/${file.id}/view`;
        ...
        <>
            ...
            <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 ${isFullscreen ? 'p-0' : ''}`}>
                <div className={`fullscreen-modal bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden animate-slideIn ${isFullscreen ? 'w-screen h-screen rounded-none' : ''}`}>
                    ...
                    <div className="bg-white border-b border-gray-200 px-6 py-4">
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-gray-500 text-sm">Visualizador de Imagem</p>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-700 text-sm min-w-[50px] text-center">{zoom}%</span>
                                <button
                    ...
                    {/* Image Container */}
                    <div 
                        className="p-6 bg-gray-100 overflow-auto flex justify-center items-center"
                        style={{ 
                            height: isFullscreen ? 'calc(100vh - 120px)' : 'calc(90vh - 120px)',
                            maxHeight: isFullscreen ? 'calc(100vh - 120px)' : 'calc(90vh - 120px)'
                        }}
                    >
                        <div className="text-center">
                            ...
                            />
                            {file.size && (
                                <p className="text-gray-500 text-sm mt-4">Tamanho: {file.size}</p>
                            )}
                        </div>
                    </div>
                </div>
