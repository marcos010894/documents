...
const EditBusnessBasic = () => {
    ...
    if (user) {
      try {
        ...
        setFormData({
          ...
          autorizacoes: info.empresa?.autorizacoes || '',
          certificados: info.empresa?.certificados || '',
          marketplace: info.empresa?.marketplace || false,
          password_user: info.user?.password_user || '',
          profileImage: info.user?.profileImage || null,
        });
        setImagePreview(info.user?.profileImage || null);
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    const personPJDate = {
      ...
      cidade: formData.cidade_empresa,
      bairro: formData.bairro,
      estado: formData.estado_empresa,
      marketplace: formData.marketplace || true,
      ...
      autorizacoes: formData.autorizacoes,
  ...
  return (
      ...
      <div className="flex items-center space-x-6 mb-8 relative">
        <div className="relative">
          ...
          <button
            ...
            className="absolute bottom-2 right-2 bg-gradient-to-r from-red-500 to-red-700 text-white rounded-full p-[6px] hover:scale-110 transition-transform"
          >
            <Camera size={20} />
          </button>
          <input
        ...
        <div>
          <p className="text-lg font-semibold text-gray-800">{formData.nome}</p>
    ...
    export default EditBusnessBasic;
